...
/* ****** 资源创建策略对象 ****** */
let srcStrategy = {
  image: function (fileSrc, eventFunc) {
    let img = new Image()
    if (eventFunc) {
  ...
  sound: function (fileSrc, eventFunc) {
    let sound = new Audio()
    if (eventFunc) {
  ...
  type: function (fileSrc, type) {
    switch (type) {
      case 'image':
        return config.imgPath + fileSrc
      case 'sound':
        return config.soundPath + fileSrc
      default:
        return ''
    }
  }
...
export default class Source {
  ...
  getSrc (fileSrc, type) { // 获取资源
    if (!this.srcBuffer.fileSrc) {
      this.srcBuffer.fileSrc = srcStrategy.type(fileSrc, type)
    }
